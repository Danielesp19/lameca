...
const DARK  = "#060402";
// Verde de WhatsApp, aclarado para que se lea sobre el fondo casi negro del pie.
const WHATSAPP = "#5FD07A";
...
// "wa" es el mismo trazo que usa el botón flotante de WhatsApp: así el ícono
// del pie y el del botón se leen como lo mismo.
function SocialIcon({ icon }: { icon: "ig" | "fb" | "tt" | "wa" }) {
  const paths = {
            ...
            {sedes.filter(s => s.whatsapp_phone).map(s => (
              <a
                ...
                style={{
                  ...
                  color: CREAM, textDecoration: "none", fontSize: 13, opacity: 0.85,
                }}
              >
                {/* El ícono en verde de WhatsApp: deja claro de un vistazo que
                    el número abre un chat, no que es para llamar. */}
                <span style={{ color: WHATSAPP, display: "flex", flexShrink: 0 }} aria-hidden="true">
                  <SocialIcon icon="wa" />
                </span>
